...
@Component({
  ...
  <button mat-raised-button type="button" style="color: white;margin-left: 15%;background-image: linear-gradient(to top, #0ba360 0%, #3cba92 100%);" class="accordionButton" (click)="showStandard();$event.stopPropagation()"><i class="fa fa-plus"></i>Standard</button>    
  <button mat-raised-button type="button"    style="color: white;margin-left:3%;background-image: linear-gradient(to top, #0ba360 0%, #3cba92 100%);" class="accordionButton" (click)="showControl();$event.stopPropagation()"><i class="fa fa-plus"></i> Control</button>    
...
<dd class="{{uncollapsed ? 
  ...
  <app-standards-form [id$] ="id$" id="standard" style="width:100%; position:absolute; display:none">
  </app-standards-form>
...
//toggle for open and close appearances
  ...
  showStandard(){
    ...
    this.state.currentPage = "standards"
